    ...
    <div class="nav3">
      ...
      <div class="myTable">
        <vxe-table :tree-config="{ children: 'category' }" :data="tableData">
          ...
          <vxe-table-column field="real_name" title="用户名"></vxe-table-column>
          <vxe-table-column field="answer" min-width="120" title="选择题答案"></vxe-table-column>
          <vxe-table-column field="content" min-width="120" title="填空题答案"></vxe-table-column>
          <vxe-table-column field="start_time" min-width="120" title="提交时间"></vxe-table-column>
          <!-- <vxe-table-column title="操作状态" width="200">
            <template slot-scope="scope">
              <el-button size="small" @click="addShop(scope.row)" type="text">添加商品</el-button>
              <el-button size="small" type="text" @click="seeShop(scope.row)">查看商品</el-button>
              <template>
                <el-button
                  ...
                  type="text"
                >删除</el-button>
                <el-button v-else size="small" @click="delZhutiFenlei(scope.row)" type="text">删除</el-button>
              </template>
            </template>
          </vxe-table-column> -->
        </vxe-table>
    ...
    >
      <div class="myForm">
        <el-form :inline="true" :model="searchAddForm" class="demo-form-inline">
          <el-form-item show-overflow="title" size="small" label="分类名称：">
            <el-input v-model="searchAddForm.keyword" placeholder="请输入"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button size="small" type="primary" @click="searchAddOnSubmit">查询</el-button>
      ...
      <div class="myTable">
        ...
        >
          ...
          <vxe-table-column field="id" title="ID"></vxe-table-column>
          <vxe-table-column tree-node field="cate_name" title="分类名称"></vxe-table-column>
      ...
      <div class="myTable">
        <vxe-table :data="tableData3">
          ...
          <vxe-table-column field="role" title="商品图">
            <template slot-scope="scope">
              <el-image
                :src="scope.row.product.product_avatar"
                fit="fill"
                style="width: 40px; height: 40px"
              >
                <div slot="error" class="image-slot">
          ...
          <vxe-table-column field="product.product_name" title="商品名称"></vxe-table-column>
          ...
          <vxe-table-column title="操作" width="80">
            <template slot-scope="scope">
              <div class="flex">
                <el-button size="small" type="text" @click="tabDelShop(scope.row)">删除</el-button>
              </div>
            </template>
          </vxe-table-column>
        </vxe-table>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.wenquan({
      ...
      this.tableData.forEach(ele => {
        ele.start_time = this.formatDate(ele.answer_time * 1000);
      });
    },
    formatDate(date1) {
